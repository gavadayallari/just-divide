        ...
        >
          {keepValue !== 0 && (
            <div
              ...
              onPointerDown={(e) => onKeepPointerDown && onKeepPointerDown(e, keepValue, 'keep')}
              draggable
              onDragStart={(e) => {
                try { 
                  e.dataTransfer.setData('application/json', JSON.stringify({ value: keepValue, source: 'keep' })); 
                } catch {}
        ...
        <div id="queue-stack" className="queue-stack">
          {queue.map((val, idx) => (
            <div
              ...
              onPointerDown={(e) => idx === 0 && onQueuePointerDown && onQueuePointerDown(e, val, 'queue')}
              draggable={idx === 0}
              onDragStart={(e) => {
                if (idx !== 0) return;
                try { 
                  e.dataTransfer.setData('application/json', JSON.stringify({ value: val, source: 'queue' })); 
                } catch {}
